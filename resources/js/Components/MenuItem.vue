    ...
    <li class="relative px-2 py-1">
        <div class="inline-flex items-center justify-between w-full text-base font-semibold transition-colors duration-150 text-gray-500 hover:text-yellow-400 cursor-pointer" @click="toggleOpen">
            <span :class="['inline-flex items-center text-sm font-semibold', $page.props.styles.textos]">
                ...
                <span class="ml-4">{{ menu.nombre }}</span>
            </span>
        ...
        <div v-if="menu.acciones.length && opened">
            <ul :class="['p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium rounded-md shadow-inner', $page.props.styles.fondo]" aria-label="submenu">
                <li class="px-2 py-1 text-white transition-colors duration-150" v-for="(accion, index) in menu.acciones.filter(m => m.esMenu)" :key="index">
                    <div :class="['px-1 hover:text-gray-800 hover:bg-gray-100 rounded-md', page.url == accion.ruta ? '!text-gray-800 !bg-gray-100' : '']">
                        <div class="flex items-center">
                            ...
                            <NavLink :href="accion.ruta">
                                <div :class="['w-full ml-2 text-sm font-semibold text-white hover:text-gray-800', page.url == accion.ruta ? '!text-gray-800' : '']">
                                    {{ accion.nombre }}
                                </div>
                            </NavLink>
                        </div>
...
import { usePage } from '@inertiajs/vue3';
    ...
    const opened = ref(props.menu.acciones.some(accion => accion.esMenu && accion.ruta == page.url))
