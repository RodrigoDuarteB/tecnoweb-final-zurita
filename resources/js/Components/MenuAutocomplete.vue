    ...
    <div>
        ...
        <ul v-if="showSuggestions" class="absolute top-12 border bg-white mt-1 shadow-xl w-[500px] rounded-md">
            ...
            >
                ...
                <NavLink  v-for="(accion, i) in item.acciones" :key="i" class="p-2 hover:bg-gray-200 cursor-pointer border-b border-b-gray-300" :href="`http://mail.tecnoweb.org.bo/inf513/grupo21sa/proyecto2/public${accion.url}`">
                    {{ accion.nombre }}
                </NavLink>
                <!-- <NavLink
...
import NavLink from './NavLink.vue';
    ...
    const fetchSuggestions = async () => {
        ...
        if (searchTerm.value.trim().length > 1) {
            ...
            try {
                const url = 'http://mail.tecnoweb.org.bo/inf513/grupo21sa/proyecto2/public/api/menu/buscar'
