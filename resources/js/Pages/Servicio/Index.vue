<template>
    <ManagmentLayout title="Lista de Servicios">
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100" v-for="item in items" :key="item">
                    ...
                    <td class="py-3 px-6 text-center">
                        <div class="flex item-center justify-center">
                            <AuthPermission menu="Gestionar Servicios" accion="Ver">
                                <VerButton :href="route('servicio.show', { id: item.id })" />
                            </AuthPermission>

                            <AuthPermission menu="Gestionar Servicios" accion="Editar">
        ...
        <DialogModal :show="showModal" @close="showModal = false">
            ...
            <template v-slot:content>
                <p>¿Estás seguro de que quieres eliminar el Servicio con id <strong> {{ selectedItem }}</strong>?</p>
            </template>
...
import { hideLoading, showLoading } from '@/state';
    ...
    const deleteItem = () => {
        if (selectedItem.value) {
            showLoading()
            router.delete(route('servicio.destroy', { id: selectedItem.value }), {
