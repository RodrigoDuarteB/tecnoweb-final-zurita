<template>
    <ManagmentLayout title="Lista de Descuentos">
        <table class="min-w-max w-full table-auto">
            <thead>
                <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                    ...
                    <th class="py-3 px-6 text-center">Porcentaje</th>
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100" v-for="item in items" :key="item">
                    ...
                    <td class="py-3 px-6 text-center">
                        <div class="flex item-center justify-center">
                            <AuthPermission menu="Gestionar Descuentos" accion="Ver">
                                <VerButton :href="route('descuento.show', { id: item.id })" />
                            </AuthPermission>

                            <AuthPermission menu="Gestionar Descuentos" accion="Editar">
                                <EditarButton :href="route('descuento.edit', { id: item.id })" />
                            </AuthPermission>

                            <AuthPermission menu="Gestionar Descuentos" accion="Eliminar">
        ...
        <DialogModal :show="showModal" @close="showModal = false">
            ...
            <template v-slot:content>
                <p>¿Estás seguro de que quieres eliminar el Descuento con id <strong> {{ selectedItem }}</strong>?</p>
            </template>
...
import { hideLoading, showLoading } from '@/state';
    ...
    const deleteItem = () => {
        if (selectedItem.value) {
            showLoading()
            router.delete(route('descuento.destroy', { id: selectedItem.value }), {
